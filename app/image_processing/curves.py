"""Curves and CUBE LUT operations -- PCHIP interpolation, 3D LUT.

Ported from master-merlin/mrln-arcane-tuner (Apache-2.0), trimmed to the
operations this trainer's dataset-adjust panel actually exposes -- no
.cube export, no 1D-LUT path. Same algorithms, unchanged.
"""
...
def apply_lut_cube(img: Image.Image, lut: CubeLUTData, strength: float = 1.0) -> Image.Image:
    ...
    # after reshape(size, size, size, 3) the table is indexed [b, g, r].
