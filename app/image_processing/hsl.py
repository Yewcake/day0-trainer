...
Ported from master-merlin/mrln-arcane-tuner (Apache-2.0), unchanged.
"""
...
def apply_hsl_selective(
    ...
    for range_name, adj in adjustments.items():
        ...
        s_adj = adj.get("saturation", 0.0) / 100.0
        l_adj = adj.get("luminance", 0.0) / 100.0
        ...
        weight *= (sat > 0.01).astype(np.float32)

        hue += h_shift * weight
        sat += s_adj * weight
        lum += l_adj * 0.5 * weight
