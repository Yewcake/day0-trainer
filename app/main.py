...
def create_job(payload: dict) -> dict:
    ...
    if model_entry["arch"] == "ideogram4":
        ...
        cmd = [
            ...
            "--quant_source", str(config["quant_source"]),
            "--dataset_dir", str(dataset_path),
            "--output_dir", str(directory),
            "--run_name", "run",
            "--trigger_word", trigger,
...
def run_analysis(job_id: str) -> dict:
    ...
    parts: list[dict] = [{
        "text": (
            ...
            "The config's learning_rate is the schedule's PEAK value, not a constant -- "
            "lr_scheduler and warmup_steps determine how it actually moves over the run (cosine "
            "and linear decay toward ~0 by the final step; constant holds after warmup). Work out "
            "the effective LR at the step where any degradation actually starts before blaming or "
            "recommending a change to the LR itself -- a schedule that's already decayed low by "
            "then points at something else (steps, dataset, repeats), not the LR. "
